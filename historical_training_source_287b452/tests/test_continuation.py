...
def test_default_full_run_stops_at_registered_cap() -> None:
    assert list(epoch_iterator(
        committed_epoch=298, registered_max_epochs=300, mode="full",
...
def test_uniform_cap_does_not_stretch_the_frozen_lr_decay_horizons() -> None:
    for spec in TASKS.values():
        ...
        assert linear_lr_multiplier(spec.lr_decay_epochs, spec.lr_decay_epochs) == pytest.approx(0.1)
