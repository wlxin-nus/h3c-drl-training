...
from drl_multiseed.observations import PolicyObservationBuilder
...
def test_protocol_identity_is_seed_independent_but_run_identity_is_not() -> None:
    for spec in TASKS.values():
        seed_42 = spec.payload(42)
        seed_1337 = spec.payload(1337)
        assert seed_42.pop("seed") == 42
        assert seed_1337.pop("seed") == 1337
        assert seed_42 == seed_1337 == spec.protocol_payload()
        assert spec.scientific_hash(42) != spec.scientific_hash(1337)
...
def test_ppo_and_mappo_share_the_same_case_observation_semantics() -> None:
    for ppo_key, mappo_key in (
        ...
        assert {key: value for key, value in ppo.items() if key not in ignored} == {
