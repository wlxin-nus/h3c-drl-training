"""Canonical observation builder for all five DRL training tasks."""
...
from .protocol import zone_temperature_c
...
def symmetric_minmax(
    ...
    """Map configured physical bounds to [-1, 1] using float32 arithmetic."""
    ...
    return np.asarray(
        2.0 * (clean - lower_f32) / (upper_f32 - lower_f32) - 1.0,
        dtype=np.float32,
    )
        ...
        if self.local_dimension:
            ...
            for zone_index, zone in enumerate(self.zone_order):
                action_columns = (
                    ...
                    else [f"action_{zone}_{index}" for index in range(self.action_history_steps)]
                )
                zone_columns = [
