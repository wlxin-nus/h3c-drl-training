...
def training_should_start(*, mode: str, committed_epoch: int, stopped: bool) -> bool:
    """Return whether a trainer should enter another epoch transaction."""

    if mode not in {"smoke", "full"}:
        raise ValueError(f"unsupported run mode: {mode}")
    if committed_epoch < 0:
        raise ValueError("committed_epoch cannot be negative")
    return not stopped and not (mode == "smoke" and committed_epoch >= 2)
