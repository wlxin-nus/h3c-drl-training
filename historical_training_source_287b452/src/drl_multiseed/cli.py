...
from .config import TASKS, get_task, repository_root
...
from .preflight import H3C_SOURCE_COMMIT, run_preflight


def _output_root(value: str | None) -> Path:
    return Path(value).resolve() if value else repository_root() / "outputs_refine_v2"
...
def _code_fingerprint() -> str:
    ...
    relative = (
        "src/drl_multiseed/config.py", "src/drl_multiseed/environment.py",
        "src/drl_multiseed/observation_contract.py",
        "src/drl_multiseed/networks.py", "src/drl_multiseed/gae.py",
        "src/drl_multiseed/early_stop.py", "src/drl_multiseed/continuation.py",
        "src/drl_multiseed/evaluation.py",
        "src/drl_multiseed/ppo_training.py", "src/drl_multiseed/mappo_training.py",
    ...
    for name in relative:
        ...
        digest.update(portable_content.encode("utf-8"))
    return digest.hexdigest()


_CONTINUATION_UPGRADE_SOURCE_FINGERPRINTS = {
    "a62c0ba840ee697b4fca02a5226899afd1573ac4de093035ed8a3b46f58defb5",
}
_CONTINUATION_UPGRADE_TARGET_FINGERPRINT = (
    "f81a60071281721e3a7e3620cd36f2e6da3fded4044ffed10456f1139f7a71e9"
)


def _identity(
    run_dir: Path, task: str, seed: int, allow_host_migration: bool,
    allow_continuation_upgrade: bool = False,
) -> dict[str, Any]:
    ...
    if path.exists():
        ...
        if fingerprint_changed and latest_checkpoint.exists():
            compatible_upgrade = (
                allow_continuation_upgrade
                and recorded_fingerprint in _CONTINUATION_UPGRADE_SOURCE_FINGERPRINTS
                and current_fingerprint == _CONTINUATION_UPGRADE_TARGET_FINGERPRINT
            )
            if not compatible_upgrade:
                raise RuntimeError(
                    "Training code identity changed after training progress was committed; "
                    "refusing silent resume. "
                    "The continuation switch only permits the audited cap-extension implementation."
                )
            append_jsonl(run_dir / "code_upgrades.jsonl", {
                "timestamp": utc_now(), "event": "audited_continuation_upgrade",
                "from_commit": recorded_commit, "to_commit": current_commit,
                "from_fingerprint": recorded_fingerprint,
            ...
            identity["continuation_upgrade_applied_at"] = utc_now()
            ...
            append_jsonl(run_dir / "code_upgrades.jsonl", {
                "timestamp": utc_now(), "event": "runtime_only_code_update",
                "from_commit": recorded_commit, "to_commit": current_commit,
            ...
            append_jsonl(run_dir / "identity_refresh.jsonl", {
                "timestamp": utc_now(),
        ...
        if original and original != hostname:
            ...
            append_jsonl(run_dir / "host_migrations.jsonl", {
                "timestamp": utc_now(), "from": original, "to": hostname,
    ...
    identity = {
        ...
        "task": task, "seed": seed, "hostname": hostname,
        "h3c_source_commit": H3C_SOURCE_COMMIT, "created_at": utc_now(),
...
def _record_http_recovery(run_dir: Path, **values: Any) -> None:
    append_jsonl(run_dir / "http_auto_resume.jsonl", {
        "timestamp": utc_now(), "protocol": HTTP_RECOVERY_PROTOCOL_ID, **values,
    })


def _wait_for_clean_recovery(
    *, run_dir: Path, endpoint: str, timeout_seconds: float,
) -> bool:
    ...
    while True:
        ...
        if not wait_until_healthy(
            endpoint, timeout_seconds=min(remaining, 30.0), poll_seconds=5.0,
        ):
            continue
        cleanup_results = cleanup_run_testids(run_dir, endpoint)
        active = _active_testid_count(run_dir)
        _record_http_recovery(
            run_dir, event="cleanup_probe", active_testids=active,
...
def _preflight_with_http_recovery(
    ...
    while True:
        try:
            run_preflight(
                task=args.task, seed=args.seed, endpoint=args.endpoint, online=True,
                output=run_dir / "preflight.json",
            )
            return
        except BaseException as exc:
            if (
                not is_retryable_boptest_error(exc)
                or attempt >= args.http_auto_resume_attempts
            ):
                raise
            attempt += 1
            ...
            _record_http_recovery(
                run_dir, event="preflight_retry_scheduled", attempt=attempt,
                error_type=type(exc).__name__, error=str(exc), delay_seconds=delay,
            )
            time.sleep(delay)
            if not wait_until_healthy(
                args.endpoint, timeout_seconds=args.http_health_timeout_seconds,
            ):
                _record_http_recovery(
                    run_dir, event="preflight_health_wait_exhausted",
                    attempt=attempt,
...
def _annotate_runtime_recovery(
    run_dir: Path, *, args: argparse.Namespace, restarts: int,
) -> None:
...
def _train(args: argparse.Namespace) -> int:
    ...
    run_dir = _run_dir(root, args.mode, args.seed, args.task)
    ...
    identity = _identity(
        run_dir, args.task, args.seed, args.allow_host_migration,
        allow_continuation_upgrade=args.continue_until_converged,
    )
    manager = CapacityManager(root / "_runtime", capacity=args.worker_capacity)
    with exclusive_run_lock(run_dir):
        with manager.hold(
            task=args.task, seed=args.seed, run_uuid=identity["run_uuid"],
            slots=5, wait_seconds=args.lease_wait_seconds,
        ):
            ...
            while True:
                kwargs = dict(
                    seed=args.seed, mode=args.mode, endpoint=args.endpoint,
                    run_dir=run_dir, resume=resume, wandb_mode=args.wandb_mode,
                    device=args.device,
                ...
                try:
                    ...
                    _annotate_runtime_recovery(
                        run_dir, args=args, restarts=total_restarts,
                    )
                    if total_restarts:
                        _record_http_recovery(
                            run_dir, event="training_recovered",
                    ...
                    can_retry = (
                        retryable
                        and consecutive_failures <= args.http_auto_resume_attempts
                    )
                    failure = {
                        "task": args.task, "seed": args.seed,
                        "timestamp": utc_now(), "error_type": type(exc).__name__,
                        "error": str(exc), "committed_epoch": committed_epoch,
                    ...
                    delay = recovery_delay(
                        consecutive_failures, args.http_resume_backoff_seconds,
                    )
                    _record_http_recovery(
                        run_dir, event="training_retry_scheduled",
                    ...
                    while True:
                        ...
                        if _wait_for_clean_recovery(
                            run_dir=run_dir, endpoint=args.endpoint,
                            timeout_seconds=args.http_health_timeout_seconds,
                        ):
                            break
                        consecutive_failures += 1
                        if consecutive_failures > args.http_auto_resume_attempts:
                            atomic_json(run_dir / "last_failure.json", {
                                **failure,
                                "timestamp": utc_now(),
                        ...
                        _record_http_recovery(
                            run_dir, event="health_wait_retry_scheduled",
                            attempt=consecutive_failures,
                            total_restarts=total_restarts,
                            committed_epoch=committed_epoch,
                            delay_seconds=delay,
...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    spec = get_task(args.task)
    run_dir = _run_dir(_output_root(args.output_root), args.mode, args.seed, args.task)
    report = evaluate_best(
        spec, seed=args.seed, endpoint=args.endpoint, run_dir=run_dir,
    ...
    try:
        while queue or active:
            while queue and len(active) < args.max_parallel:
                ...
                command = [
                    sys.executable, "-m", "drl_multiseed.cli", "train",
                    "--task", task, "--seed", str(args.seed), "--mode", args.mode,
                    "--endpoint", args.endpoint, "--output-root", str(root),
                    "--wandb-mode", args.wandb_mode, "--device", device,
                    "--worker-capacity", str(args.worker_capacity),
                    "--lease-wait-seconds", "86400",
                    "--http-auto-resume-attempts", str(args.http_auto_resume_attempts),
                    "--http-resume-backoff-seconds", str(args.http_resume_backoff_seconds),
                    "--http-health-timeout-seconds", str(args.http_health_timeout_seconds),
                ]
                if args.resume:
                ...
                process = subprocess.Popen(command, cwd=repository_root(), env=environment, text=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="H3C refined-observation DRL multi-seed runner")
    ...
    preflight.set_defaults(func=lambda a: (print(json.dumps(run_preflight(
        task=a.task, seed=a.seed, endpoint=a.endpoint, online=a.online,
        output=_output_root(a.output_root) / "preflight.json",
    ), indent=2)), 0)[1])

    aggregate = sub.add_parser("aggregate", parents=[common])
    aggregate.set_defaults(func=lambda a: (print(json.dumps(
        aggregate_results(_output_root(a.output_root), a.mode), indent=2
    )), 0)[1])
