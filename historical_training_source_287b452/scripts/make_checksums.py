...
EXCLUDED_PARTS = {".git", ".pytest_cache", ".ipynb_checkpoints", "__pycache__"}
EXCLUDED_ROOT_DIRECTORIES = {
    "outputs", "outputs_refine", "outputs_refine_uniform", "outputs_refine_v2",
    "runtime", "wandb"
}
...
def included(path: Path) -> bool:
    ...
    return (
        ...
        and relative.parts[:2] != ("models", "generated")
...
def main() -> None:
    ...
    if args.verify:
        ...
        changed = sorted(name for name in set(recorded) & set(values) if recorded[name] != values[name])
        if missing or added or changed:
            raise SystemExit(f"Checksum failure: missing={missing}, added={added}, changed={changed}")
