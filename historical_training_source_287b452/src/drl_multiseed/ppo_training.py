...
def _factory(
    spec: TaskSpec, seed: int, forecast: Mapping[str, Sequence[float]],
    endpoint: str, run_dir: Path, rank: int,
) -> gym.Env:
...
def evaluate_train_window(
    model: Any, spec: TaskSpec, seed: int, forecast: Mapping[str, Sequence[float]],
    endpoint: str, run_dir: Path, epoch: int,
) -> dict[str, Any]:
    env = make_env(
        spec, seed, forecast, endpoint, run_dir, "eval",
        f"train_week_eval_{epoch:04d}", evaluation=False,
    )
    rewards: list[float] = []
    ...
    _write_rows(
        ...
        [{"step": index, "reward": reward, "cost": costs[index], "actions": json.dumps(action_rows[index])}
         for index, reward in enumerate(rewards)],
    )
    actions = np.asarray(action_rows, dtype=float)
    return {
        "epoch": int(epoch), "return": float(np.sum(rewards)), "cost": float(np.sum(costs)),
...
class TransactionalPPOCallback(BaseCallback):
    def __init__(
        self, *, spec: TaskSpec, seed: int, run_dir: Path, mode: str,
        endpoint: str, forecast: Mapping[str, Sequence[float]],
        checkpoint: checkpoint_core.AtomicCheckpointManager,
        tracker: SafeWandb, restored: Mapping[str, Any] | None,
        environment_fingerprint: str, continue_until_converged: bool,
    ):
    ...
    def _on_step(self) -> bool:
        ...
        for index, info in enumerate(infos):
            ...
            pmvs = info.get("pmvs", [])
            for action, occupancy, pmv in zip(actions, occupancies, pmvs):
    ...
    def _state(self, epoch: int) -> dict[str, Any]:
        return {
            "schema": "h3c-drl-multiseed-refined-ppo-state-v1",
            "config_hash": self.spec.scientific_hash(self.seed),
            "observation_contract": OBSERVATION_CONTRACT_ID,
            "environment_fingerprint": self.environment_fingerprint,
            "committed_epoch": int(epoch), "global_step": int(self.model.num_timesteps),
            "training_rows": self.training_rows, "update_rows": self.update_rows,
            "evaluation_rows": self.evaluation_rows, "early_stop": self.plateau.to_dict(),
            "best_score": None if not math.isfinite(self.plateau.best_score) else self.plateau.best_score,
            "best_epoch": self.plateau.best_epoch, "resume_count": self.resume_count,
            ...
            "wandb_run_id": self.tracker.run_id, "updated_at": utc_now(),
            "code_commit": _run_identity(self.run_dir, self.spec, self.seed).get("code_commit"),
            "code_fingerprint": _run_identity(self.run_dir, self.spec, self.seed).get("code_fingerprint"),
        }
    ...
    def on_post_update(self, model: Any) -> bool:
        ...
        train_row = {
            "run_uuid": self.tracker.run_id, "epoch": epoch,
            "global_step": int(model.num_timesteps), "reward_mean": reward_mean,
            ...
            "occupied_action_saturation": float(np.mean(np.abs(self.occupied_actions) > 0.95)) if self.occupied_actions else 0.0,
            "action_occ_unocc_gap": float(abs(np.mean(self.occupied_actions) - np.mean(self.unoccupied_actions))) if self.occupied_actions and self.unoccupied_actions else 0.0,
            "occupied_pmv_violation_rate": float(np.mean(np.abs(self.occupied_pmv) > self.spec.comfort_threshold)) if self.occupied_pmv else 0.0,
            "epoch_wall_seconds": time.perf_counter() - self._started, "timestamp": utc_now(),
        }
        logger = getattr(model.logger, "name_to_value", {})
        gradient_norm = float(torch.sqrt(sum(
            torch.sum(parameter.grad.detach() ** 2)
            for parameter in model.policy.parameters() if parameter.grad is not None
        )).cpu()) if any(parameter.grad is not None for parameter in model.policy.parameters()) else float("nan")
        update_row = {
            "run_uuid": self.tracker.run_id, "epoch": epoch,
        ...
        if epoch % interval == 0:
            evaluation = evaluate_train_window(
                model, self.spec, self.seed, self.forecast, self.endpoint, self.run_dir, epoch
            )
            stop_update = self.plateau.update(epoch, float(evaluation["return"]))
            evaluation.update(stop_update)
            self.evaluation_rows.append(evaluation)
        ...
        self.tracker.log({
            "epoch": epoch, "train/reward_mean": reward_mean,
            ...
            "train_window/return": self.evaluation_rows[-1]["return"] if self.evaluation_rows and self.evaluation_rows[-1]["epoch"] == epoch else float("nan"),
...
def _run_identity(run_dir: Path, spec: TaskSpec, seed: int) -> dict[str, Any]:
    ...
    identity = {"run_uuid": f"{spec.key}-seed{seed}-{uuid.uuid4().hex[:12]}", "created_at": utc_now()}
    atomic_json(path, identity)
    return identity


def train_ppo(
    spec: TaskSpec, *, seed: int, mode: str, endpoint: str,
    ...
    tracker = SafeWandb(
        run_dir, project=WANDB_PROJECT, group=f"seed{seed}",
        name=f"{spec.key}-seed{seed}-{mode}", run_id=identity["run_uuid"],
    ...
    try:
        ...
        if latest:
            ...
            model = checkpoint_core.Phase15fPPO.load(
                latest["model_path"], env=vector, device=device,
            ...
            model = checkpoint_core.Phase15fPPO(
                "MlpPolicy", vector, learning_rate=schedule, n_steps=spec.n_steps,
                batch_size=spec.batch_size, n_epochs=spec.n_epochs, gamma=spec.gamma,
                gae_lambda=spec.gae_lambda, clip_range=spec.clip_range,
                ent_coef=spec.ent_coef, vf_coef=spec.vf_coef,
                max_grad_norm=spec.max_grad_norm, policy_kwargs=policy_kwargs,
                seed=seed, device=device, verbose=1,
                tensorboard_log=str(run_dir / "tensorboard"),
            )
            restored = None
        callback = TransactionalPPOCallback(
            spec=spec, seed=seed, run_dir=run_dir, mode=mode, endpoint=endpoint,
            forecast=forecast, checkpoint=checkpoint, tracker=tracker,
            restored=restored, environment_fingerprint=fingerprint,
        ...
        if remaining > 0 and not callback.plateau.stopped:
        ...
        tracker.finish({
            ...
            "early_stopped": False if final is None else final["state"]["early_stop"]["stopped"],
        })
    final = checkpoint.load_latest_metadata()
    if final is None:
    ...
    atomic_json(run_dir / "run_manifest.json", {
        "schema": "h3c-drl-multiseed-refined-run-v1", "task": spec.key, "seed": seed,
        "mode": mode, "config_hash": config_hash, "run_uuid": identity["run_uuid"],
        "status": completion_status(stopped=stopped, reached_cap=completed_epoch >= spec.max_epochs),
        "completed_epoch": completed_epoch,
        "global_step": final["state"]["global_step"],
        "best_epoch": final["state"]["best_epoch"],
        "best_score": final["state"]["best_score"], "updated_at": utc_now(),
        "selection_basis": "highest deterministic training-window return at registered validation points",
        "held_out_used_for_selection": False, "training_environments": 4,
        "observation_dimension": spec.observation_dim, "action_dimension": spec.action_dim,
        "observation_contract": OBSERVATION_CONTRACT_ID,
        ...
        "code_commit": identity.get("code_commit"), "code_fingerprint": identity.get("code_fingerprint"),
