...
class LifecycleRecorder:
    ...
    def __call__(self, event: Mapping[str, Any]) -> None:
        row = {
            "timestamp": utc_now(), "hostname": os.environ.get("COMPUTERNAME", "unknown"),
            "pid": os.getpid(), "owner": self.owner, "task": self.task,
            "seed": self.seed, **dict(event),
        }
...
def _training_objective(profile: Mapping[str, Any], spec: TaskSpec) -> dict[str, float]:
    ...
    # 0.2 * 0.193466; the H3C profile stores 0.1 * 0.386932.
    return objective


def training_reward(
    *, cost: float, pmv: Sequence[float], occupancy: Sequence[float],
    setpoints: Sequence[float], previous: Sequence[float],
    objective: Mapping[str, float], comfort_threshold: float,
) -> tuple[float, dict[str, float]]:
    zones = len(pmv)
    comfort = sum(
        max(0.0, abs(float(value)) - comfort_threshold) ** 2
        if float(count) > 0 else 0.0
...
def prefetch_forecast(
    ...
    try:
        ...
        resolved, _events = resolve_forecast_missing_occupancy(
            profile, source, forecast_points(profile), required,
            forecast_phase="evaluation" if evaluation else "training",
            start_time_seconds=start_day * 86400, step_seconds=900,
        )
        return {key: [float(value) for value in values[:required]] for key, values in resolved.items()}
    finally:
        if client.test_id is not None:
            client.stop()


class H3CCompatibleTrainingEnv(gym.Env[np.ndarray, np.ndarray]):
    """BOPTEST environment using the preregistered refined observation contract.

    Legacy model metadata stays read-only. New training and paper evaluation
    share this exact refined column order, normalization and history contract.
    """

    metadata = {"render_modes": []}

    def __init__(
        self, spec: TaskSpec, seed: int, forecast: Mapping[str, Sequence[float]],
        endpoint: str, run_dir: Path, rank: int | str, phase: str,
        *, evaluation: bool = False,
        ...
        self.profile = load_profile(CASE_NAMES[spec.case_key])
    ...
    def _current_occupancy(self, step: int) -> dict[str, float]:
        ...
        return {
            zone: effective_count(
                self.profile["occupancy"], action_time,
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self.client.test_id is None:
            self.state = self.client.initialize(self.spec.case_name, self._start_seconds(), 7 * 86400)
        else:
    ...
    def step(self, action: np.ndarray):
        ...
        for index, zone in enumerate(self.zone_order):
            if self.model_contract["residual_base"] == "fixed_25":
                base = 25.0
            else:
                base = 25.0 if occupancy[zone] > 0 else 30.0
            setpoints[zone] = max(20.0, min(30.0, base + 5.0 * float(actions[index])))
        ...
        # Formal H3C evaluation owns the common |PMV| > 0.5 KPI/reward
        # threshold.  Training keeps the archived case-specific threshold so
        # that a new seed changes no scientific setting other than the seed
        # and the preregistered stopping rule.
        comfort_threshold = 0.5 if self.evaluation else self.spec.comfort_threshold
        reward, components = training_reward(
            cost=cost, pmv=[pmv[z] for z in self.zone_order],
            ...
            objective=self.objective, comfort_threshold=comfort_threshold,
        )
        ...
        info = {
            ...
            "actions": actions.tolist(), "setpoints": [setpoints[z] for z in self.zone_order],
            "occupancies": [occupancy[z] for z in self.zone_order],
            "temperatures": [temperatures[z] for z in self.zone_order],
            "pmvs": [pmv[z] for z in self.zone_order], "power_total": power,
            "phys_cost": cost, "price": price, **components,
...
def make_env(
    spec: TaskSpec, seed: int, forecast: Mapping[str, Sequence[float]], endpoint: str,
    run_dir: Path, rank: int | str, phase: str, *, evaluation: bool = False,
) -> H3CCompatibleTrainingEnv:
    return H3CCompatibleTrainingEnv(
