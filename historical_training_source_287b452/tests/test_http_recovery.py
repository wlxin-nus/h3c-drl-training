...
def test_bare_checkpoint_eof_and_http_404_are_not_retried() -> None:
    ...
    assert not is_retryable_boptest_error(urllib.error.HTTPError(
        "http://example", 404, "not found", {}, None,
    ))


@pytest.mark.parametrize("error", [ValueError("shape"), RuntimeError("checkpoint checksum mismatch")])
...
def test_health_wait_retries_without_mutating_testcases() -> None:
    ...
    assert wait_until_healthy(
        "http://example", timeout_seconds=5, poll_seconds=1,
...
def test_train_auto_resumes_same_run_after_worker_eof(tmp_path, monkeypatch) -> None:
    ...
    args = argparse.Namespace(
        task="sz_air_ppo", seed=1337, mode="full", endpoint="http://example",
        output_root=None, worker_capacity=12, allow_host_migration=False,
        continue_until_converged=False, lease_wait_seconds=0, resume=False,
        wandb_mode="disabled", device="cpu", http_auto_resume_attempts=2,
        http_resume_backoff_seconds=1.0, http_health_timeout_seconds=30.0,
    )
    assert cli._train(args) == 0
    assert calls == [False, True]
    failure = json.loads((
        tmp_path / "full" / "seed1337" / "sz_air_ppo" / "last_failure.json"
    ).read_text(encoding="utf-8"))
    assert failure["auto_resume_scheduled"] is True


def test_identity_allows_audited_runtime_only_update_after_checkpoint(tmp_path, monkeypatch) -> None:
