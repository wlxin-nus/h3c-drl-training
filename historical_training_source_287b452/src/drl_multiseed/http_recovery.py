...
def is_retryable_boptest_error(error: BaseException) -> bool:
    ...
    for current in exception_chain(error):
        # Avoid importing the full H3C runtime in the CLI supervisor.  The
        ...
        if type(current).__name__ == "TransportError":
            if bool(getattr(current, "retryable", False)) or getattr(
                current, "error_type", None
            ) in _TRANSIENT_HTTP_TYPES:
                return True
...
def endpoint_is_healthy(endpoint: str, *, timeout_seconds: float = 10.0) -> bool:
    ...
    try:
        with urllib.request.urlopen(
            f"{endpoint.rstrip('/')}/version", timeout=timeout_seconds,
        ) as response:
