...
class SafeWandb:
    def __init__(self, run_dir: Path, *, project: str, group: str, name: str, run_id: str, mode: str, config: Mapping[str, Any]):
    ...
    def start(self) -> None:
        ...
        for variable, child in (
            ("WANDB_DATA_DIR", "data"), ("WANDB_CACHE_DIR", "cache"),
            ("WANDB_CONFIG_DIR", "config"), ("WANDB_ARTIFACT_DIR", "artifacts"),
        ):
            path = root / child
        ...
        try:
            ...
            for candidate in dict.fromkeys(modes):
                try:
                    self.run = wandb.init(
                        ...
                        id=self.run_id, resume="allow", mode=candidate,
                        dir=str(root), config=self.config, reinit=True,
            ...
            self.error = repr(exc)
        atomic_json(root / "wandb_disabled.json", {"error": self.error, "requested_mode": self.mode})
    ...
    def log(self, values: Mapping[str, Any], *, step: int) -> None:
        ...
        try:
            ...
            self.error = repr(exc)
            append_jsonl(self.run_dir / "wandb" / "pending_history.jsonl", {
                **row, "wandb_error": self.error,
            })
            self.run = None
    ...
    def finish(self, summary: Mapping[str, Any] | None = None) -> None:
        if self.run is None:
            if summary:
                atomic_json(self.run_dir / "wandb" / "pending_summary.json", {
                    "timestamp": utc_now(), **dict(summary), "wandb_error": self.error,
                })
            return
        try:
